...
async def pm_search(client, message):
    ...
    if await db.get_pm_search_status(bot_id):
        s = await message.reply(
            f"<b><i>⚠️ `{message.text}` searching...</i></b>", quote=True
        )
...
async def group_search(client, message):
    ...
    if not await db.get_chat(message.chat.id):
        ...
        username = (
            ...
            else getattr(vp, "invite_link", "N/A")
        )
    ...
    if settings.get("auto_filter"):
        ...
        if message.chat.id == SUPPORT_GROUP:
            ...
            if files:
                ...
                await message.reply_text(
                    f"Total {total_results} results found in this group",
                    reply_markup=InlineKeyboardMarkup(btn),
                )
            return
...
async def advantage_spoll_choker(bot, query):
    ...
    s = await query.message.edit_text(
        f"<b><i><code>{search}</code> Check In My Database...</i></b>"
    )
    await query.answer("")
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        if btn:
            await query.answer(
                "Kindly Join Given Channel To Get Unmute", show_alert=True
            )
        else:
        ...
        buttons = [
            ...
            [
                ...
                InlineKeyboardButton(
                    "🆕 Mᴀɪɴ Cʜᴀɴɴᴇʟ ", url=f"https://t.me/KR_PICTURE"
                ),
            ],
        ]
        ...
        await query.message.edit_text(
            script.STATUS_TXT.format(
                files, users, chats, premium, u_size, f_size, uptime
            ),
            reply_markup=InlineKeyboardMarkup(buttons),
        )
        ...
        buttons = [
            [
                InlineKeyboardButton(
                    text=f"☎️ ᴄᴏɴᴛᴀᴄᴛ - {(await client.get_users(admin)).first_name}",
                    user_id=admin,
                )
            ]
            for admin in ADMINS
        ...
        buttons = [
            [
                InlineKeyboardButton(
                    "‼️ ʜᴏᴡ ᴛᴏ ᴄᴏɴɴᴇᴄᴛ sʜᴏʀᴛɴᴇʀ ‼️", callback_data="howshort"
                )
            ],
            [InlineKeyboardButton("≼ ʙᴀᴄᴋ", callback_data="start")],
        ]
        ...
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
            text=script.USER_COMMAND_TXT, reply_markup=reply_markup
        )
        ...
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
            text=script.ADMIN_COMMAND_TXT, reply_markup=reply_markup
        )
        ...
        await query.message.edit(
            f"Deleted {deleted} files in your database in your query {query_}"
        )
        ...
        users_id = []
        await query.message.edit(
            "Unmute all started! This process maybe get some time..."
        )
        try:
            async for member in client.get_chat_members(
                query.message.chat.id, filter=enums.ChatMembersFilter.RESTRICTED
            ):
        ...
        if users_id:
            await query.message.reply(
                f"Successfully unmuted <code>{len(users_id)}</code> users."
            )
        else:
        ...
        users_id = []
        await query.message.edit(
            "Unban all started! This process maybe get some time..."
        )
        try:
            async for member in client.get_chat_members(
                query.message.chat.id, filter=enums.ChatMembersFilter.BANNED
            ):
                users_id.append(member.user.id)
            for user_id in users_id:
                await client.unban_chat_member(query.message.chat.id, user_id)
        ...
        if users_id:
            await query.message.reply(
                f"Successfully unban <code>{len(users_id)}</code> users."
            )
        else:
        ...
        users_id = []
        await query.message.edit(
            "Kick muted users started! This process maybe get some time..."
        )
        try:
            async for member in client.get_chat_members(
                query.message.chat.id, filter=enums.ChatMembersFilter.RESTRICTED
        ...
        if users_id:
            await query.message.reply(
                f"Successfully kicked muted <code>{len(users_id)}</code> users."
            )
        else:
        ...
        users_id = []
        await query.message.edit(
            "Kick deleted accounts started! This process maybe get some time..."
        )
        try:
        ...
        if users_id:
            await query.message.reply(
                f"Successfully kicked deleted <code>{len(users_id)}</code> accounts."
            )
        else:
        ...
        await query.message.edit_media(
            InputMediaPhoto(
                media=PAYMENT_QR, caption=script.PREMIUM_PLAN_TEXT.format(OWNER_UPI_ID)
            )
        )
    ...
    buttons = [
        [
            InlineKeyboardButton(
                ...
                callback_data=f"spolling#{movie.movieID}#{user}",
            )
        ]
        for movie in movies
    ]
