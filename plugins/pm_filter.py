...
async def set_shortner_2(c, m):
    ...
    try:
        URL = m.command[1]
        API = m.command[2]
        resp = requests.get(f'https://{URL}/api?api={API}&url=https://telegram.me/KR_PICTURE').json()
        if resp['status'] == 'success':
            SHORT_LINK = resp['shortenedUrl']
...
async def deletemultiplefiles(bot, message):
    ...
    btn = [[
       InlineKeyboardButton("Yᴇs, Cᴏɴᴛɪɴᴜᴇ !", callback_data=f"KR_PICTURE#{keyword}")
       ],[
