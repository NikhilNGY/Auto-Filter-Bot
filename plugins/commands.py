...
async def start(client, message):
    ...
    if (
        ...
        and datetime.datetime.now() > verify_status["expire_time"]
    ):
        await update_verify_status(message.from_user.id, is_verified=False)

    if (len(message.command) != 2) or (
        len(message.command) == 2 and message.command[1] == "start"
    ):
        buttons = [
            [
                InlineKeyboardButton(
                    "📢   Bᴀᴄᴋ Uᴘ Cʜᴀɴɴᴇʟ  📢", url=f"https://t.me/+pCz5eoun5Zk5YzRl"
                )
            ],
            [
                InlineKeyboardButton(
                    "🎞 Mᴀɪɴ Gʀᴏᴜᴘ ", url=f"https://t.me/Sandalwood_Kannada_Group"
                ),
    ...
    if mc.startswith("verify"):
        ...
        expiry_time = datetime.datetime.now() + datetime.timedelta(
            seconds=VERIFY_EXPIRE
        )
        await update_verify_status(
            message.from_user.id,
            is_verified=True,
            verified_time=time_now(),
        ...
        if verify_status["link"] == "":
            ...
            btn = [
                [
                    InlineKeyboardButton(
                        "📌 Get File 📌",
                        url=f'https://t.me/{temp.U_NAME}?start={verify_status["link"]}',
                    )
                ]
            ]
            reply_markup = InlineKeyboardMarkup(btn)
        ...
        if settings["fsub"]:
            ...
            if btn:
                btn.append(
                    [
                        InlineKeyboardButton(
                            "🔁 Try Again 🔁", callback_data=f"checksub#{mc}"
                        )
                    ]
                )
                reply_markup = InlineKeyboardMarkup(btn)
    ...
    if mc.startswith("all"):
        ...
        file_ids = []
        total_files = await message.reply(
            f"<b><i>🗂 Total files - <code>{len(files)}</code></i></b>"
        )
        for file in files:
            ...
            if settings.get("is_stream", IS_STREAM):
                btn = [
                    [
                        InlineKeyboardButton(
                            "✛ ᴡᴀᴛᴄʜ & ᴅᴏᴡɴʟᴏᴀᴅ ✛",
                            callback_data=f"stream#{file.file_id}",
                        )
                    ],
                    [
            ...
            msg = await client.send_cached_media(
                ...
                caption=f_caption,
                protect_content=(
                    False if await db.has_premium_access(message.from_user.id) else True
                ),
                reply_markup=InlineKeyboardMarkup(btn),
            )
        ...
        buttons = [
            [
                InlineKeyboardButton(
                    "ɢᴇᴛ ғɪʟᴇs ᴀɢᴀɪɴ",
                    callback_data=f"get_del_send_all_files#{grp_id}#{key}",
                )
            ]
        ]
        await client.delete_messages(
            chat_id=message.chat.id, message_ids=file_ids + [total_files.id]
        )
        await vp.edit(
    ...
    if type_ != "shortlink" and settings["shortlink"]:
        if not await db.has_premium_access(message.from_user.id):
            link = await get_shortlink(
                settings["url"],
                settings["api"],
                f"https://t.me/{temp.U_NAME}?start=shortlink_{grp_id}_{file_id}",
            )
            btn = [
                [InlineKeyboardButton("♻️ Get File ♻️", url=link)],
                [
                    InlineKeyboardButton(
                        "📍 ʜᴏᴡ ᴛᴏ ᴏᴘᴇɴ ʟɪɴᴋ 📍", url=settings["tutorial"]
                    )
                ],
            ]
            await message.reply(
    ...
    btns = [
        [
            InlineKeyboardButton(
                "ɢᴇᴛ ғɪʟᴇ ᴀɢᴀɪɴ", callback_data=f"get_del_file#{grp_id}#{file_id}"
            )
        ]
    ]
    await msg.delete()
...
async def stats(bot, message):
    ...
    await message.reply_text(
        script.STATUS_TXT.format(files, users, chats, premium, u_size, f_size, uptime)
    )
    ...
    if settings is not None:
        buttons = [
            ...
            [
                ...
                InlineKeyboardButton(
                    "⛓ Link" if settings["links"] else "🧲 Button",
                    callback_data=f'setgs#links#{settings["links"]}#{str(grp_id)}',
                ),
            ],
            [
                InlineKeyboardButton(
                    "Stream",
                    callback_data=f'setgs#is_stream#{settings.get("is_stream", IS_STREAM)}#{str(grp_id)}',
                ),
                InlineKeyboardButton(
                    "✅ On" if settings.get("is_stream", IS_STREAM) else "❌ Off",
                    callback_data=f'setgs#is_stream#{settings.get("is_stream", IS_STREAM)}#{str(grp_id)}',
                ),
            ],
...
async def save_shortlink(client, message):
    ...
    try:
        ...
        return await message.reply_text(
            "Your shortlink API or URL invalid, Please Check again!"
        )
    await save_group_settings(grp_id, "url", url)
    await save_group_settings(grp_id, "api", api)
    await message.reply_text(
        f"Successfully changed shortlink for {title} to\n\nURL - {url}\nAPI - {api}"
    )
...
async def save_welcome(client, message):
    ...
    await message.reply_text(
        f"Successfully changed welcome for {title} to\n\n{welcome}"
    )
...
async def delete_file(bot, message):
    ...
    await msg.edit(
        f"Total {total} files found in your query {query}.\n\nDo you want to delete?",
        reply_markup=InlineKeyboardMarkup(btn),
    )
...
async def set_tutorial(client, message):
    ...
    await message.reply_text(
        f"Successfully changed tutorial for {title} to\n\n{tutorial}"
    )
...
async def check_plans_cmd(client, message):
    ...
    if await db.has_premium_access(user_id):
        ...
        await message.reply_text(
            f"**Your plans details are :\n\nRemaining Time : {remaining_time}\n\nExpirytime : {expiry_time}**"
        )
    else:
        btn = [
            [
                InlineKeyboardButton(
                    "ɢᴇᴛ ғʀᴇᴇ ᴛʀᴀɪʟ ғᴏʀ 𝟻 ᴍɪɴᴜᴛᴇꜱ ☺️", callback_data="get_trail"
                )
            ],
            [
                InlineKeyboardButton(
                    "ʙᴜʏ sᴜʙsᴄʀɪᴘᴛɪᴏɴ : ʀᴇᴍᴏᴠᴇ ᴀᴅs", callback_data="buy_premium"
                )
            ],
            [InlineKeyboardButton("⚠️ ᴄʟᴏsᴇ / ᴅᴇʟᴇᴛᴇ ⚠️", callback_data="close_data")],
        ]
        ...
        m = await message.reply_sticker(
            "CAACAgIAAxkBAAIBTGVjQbHuhOiboQsDm35brLGyLQ28AAJ-GgACglXYSXgCrotQHjibHgQ"
        )
        await message.reply_text(
...
async def set_pm_search(client, message):
    ...
    try:
        ...
        return await message.reply_text(
            "<b>💔 Invalid option. Please send me 'on' or 'off' / 'true' or 'false' after the command.</b>"
        )
    if option in ["on", "true"]:
        await db.update_pm_search_status(bot_id, enable=True)
        await message.reply_text(
            "<b>✅️ ᴘᴍ ꜱᴇᴀʀᴄʜ ᴇɴᴀʙʟᴇᴅ ꜰʀᴏᴍ ɴᴏᴡ ᴜꜱᴇʀꜱ ᴀʙʟᴇ ᴛᴏ ꜱᴇᴀʀᴄʜ ᴍᴏᴠɪᴇ ɪɴ ʙᴏᴛ ᴘᴍ.</b>"
        )
    elif option in ["off", "false"]:
        await db.update_pm_search_status(bot_id, enable=False)
        await message.reply_text(
            "<b>❌️ ᴘᴍ ꜱᴇᴀʀᴄʜ ᴅɪꜱᴀʙʟᴇᴅ, ɴᴏ ᴏɴᴇ ᴜꜱᴇʀꜱ ᴀʙʟᴇ ᴛᴏ ꜱᴇᴀʀᴄʜ ᴍᴏᴠɪᴇ ɪɴ ʙᴏᴛ ᴘᴍ.</b>"
        )
    else:
        await message.reply_text(
            "<b>💔 Invalid option. Please send me 'on' or 'off' / 'true' or 'false' after the command.</b>"
...
async def set_fsub(client, message):
    ...
    await message.reply_text(
        f"Successfully set force channels for {title} to\n\n<code>{channels}</code>"
    )
...
async def remove_fsub(client, message):
    ...
    if not settings["fsub"]:
        await message.reply_text(
            "ʏᴏᴜ ᴅɪᴅɴ'ᴛ ᴀᴅᴅᴇᴅ ᴀɴʏ ꜰᴏʀᴄᴇ sᴜʙsᴄʀɪʙᴇ ᴄʜᴀɴɴᴇʟ..."
        )  # query.answer not work in command so I can change to message.reply_text
        return
